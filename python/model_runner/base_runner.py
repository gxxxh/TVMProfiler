...
from model_runner.util import WriteConfigSpace
...
class ModelRunner():
    ...
    def extractTuneTasks(self):
        if self.target == "llvm":
            tuning_option = {
                ...
                "log_filename": "/home/gh/OpBench/data/Performance/" + self.model_name + '-' + self.tuner + "-" + str(
    ...
    def getLibModuleDevDebug(self, config={}):
        ...
        module = debug_executor.create(lib.get_graph_json(), lib.get_lib(), dev,"/home/gh/TVMProfiler/data/" )
        return lib, module, dev
    ...
    def tuneOnTVM(self):
        ...
        with autotvm.apply_history_best(
                "/home/gh/OpBench/data/Performance/" + self.model_name + '-' + self.tuner + "-" + self.target + "-autotvm.json") as ab:
            # todo 如何工作的
